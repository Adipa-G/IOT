...
class WLANConfigService:
    def __init__(self):
        ...
        self.encryptionKey = bytes(wifiMac, 'utf8')

    def write_config(self, ssid, password):
        dataToWrite = (ssid + '\n' + password).encode() 
        encAlgo = ucryptolib.aes(self.encryptionKey, 1)
        encrypted = encAlgo.encrypt(dataToWrite + b' ' * ((16 - (len(dataToWrite) % 16)) % 16))
        f = None
        try:
            f = open(PASSWORD_FILE, 'w')
            f.write(encrypted)
        except Exception as e:
            self.log_service.log('error writing wifi config ' + str(e))
        finally:
            if f != None:
                f.close()
        
    def read_config(self):
        decAlgo = ucryptolib.aes(self.encryptionKey, 1)
        f = None
        try:
            ...
            self.log_service.log('error reading wifi config or config does not exists ' + str(e))
        finally:
            if f != None:
                f.close()
    ...
    def delete_config(self):
        try:
            ...
            self.log_service.log('error removing wifi config ' + str(e))

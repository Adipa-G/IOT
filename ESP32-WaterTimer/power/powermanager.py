...
class PowerManager:
    ...
    def __sleep_when_low_power(self, voltage):
        if (voltage < self.power_config['highBattery.minVoltage']):
            self.__sleep_for_duration(self.power_config['mediumBattery.deepSleepDurationUtc'])
        elif (voltage < self.power_config['mediumBattery.minVoltage']):
            self.__sleep_for_duration(self.power_config['lowBattery.deepSleepDurationUtc'])
        elif (voltage < self.power_config['lowBattery.minVoltage']):
            self.__deep_sleep(self.power_config['lowBattery.continousDeepSleepHours'] * 60 * 60 * 1000)
    ...
    def __sleep_for_duration(self, duration):
        ...
        if (current_minute_of_day >= start_minute_of_day and current_minute_of_day < end_minute_of_day):

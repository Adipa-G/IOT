...
class WLANSetup:
    ...
    def start_config_mode(self, force):
        ...
        if force == True or configValue == None:
            ...
            ap.config(essid=wifiName, password=wifiPass, authmode=4) #authmode=1 == no pass
            result = type('', (), {})()
    ...
    def __print_wifi_setup_details(self, config):
        ...
        self.screen.draw_text('ssid : ' + config.ssid, FONT_LEFT, 90, displayDriver.WHITE)
        self.screen.draw_text('password : ' + config.password, FONT_LEFT, 100, displayDriver.WHITE)
        self.screen.draw_text('open follwing url', FONT_LEFT, 120, displayDriver.WHITE)
        self.screen.draw_text('http://' + config.ipAddress, FONT_LEFT, 130, displayDriver.WHITE)
        self.screen.draw_text('press left button to', FONT_LEFT, 150, displayDriver.WHITE)

...
class PowerConfigService:
    ...
    def write_config(self, config):
        self.config_service.write_config(POWER_CONFIG_FILE, config)
        
    def read_config(self):
        cfg = self.config_service.read_config(POWER_CONFIG_FILE)
        ...
        cfg['mediumBattery.deepSleepDurationUtc'] = cfg.get('mediumBattery.deepSleepDurationUtc', '12:00-20:00')
        cfg['lowBattery.minVoltage'] = cfg.get('lowBattery.minVoltage', 3)
        cfg['lowBattery.cpuFreqMHz'] = cfg.get('lowBattery.cpuFreqMHz', 80)
        cfg['lowBattery.deepSleepDurationUtc'] = cfg.get('lowBattery.deepSleepDurationUtc', '7:00-22:00')
        cfg['extraLowBattery.continousDeepSleepHours'] = cfg.get('extraLowBattery.continousDeepSleepHours', 8)
        return cfg

const TimeUtils = {
    timeToUtc: (localTime) => {
        ...
        var localStr = `${date.getFullYear()}-${pad(date.getMonth() + 1)}-${pad(date.getDate())}T${pad(periodTokens[0])}:${pad(periodTokens[1])}:00.000`;
    ...
    timeToLocal: (utcTime) => {
        ...
        var utcStr = `${date.getFullYear()}-${pad(date.getMonth() + 1)}-${pad(date.getDate())}T${pad(periodTokens[0])}:${pad(periodTokens[1])}:00.000Z`;
